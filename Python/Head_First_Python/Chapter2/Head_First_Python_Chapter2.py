...
总结:
    pyc 文件说明：
        解释器第一次执行模块代码时，他会读入代码，并转换为一种中间字节码。Python解释器下一次使用模块时会检查模块代码文件是否有变化，如果模块代码没有改变，解释器就会跳过这个转换过程，直接运行pyc文件，如果代码确实有改变，则会重新转换pyc文件。这样做会使程序运行的快。程序运行时不能只有pyc文件。
    BIF：
        list(): 这是一个工厂函数，创建一个新的空列表
        range()：返回一个迭代器，根据需要生成一个指定范围的数字
        enumerate()：创建成对数据的一个编号列表，从0开始
        int()：将一个字符串或另一个数转换为一个整数
        id()：返回一个Python数据对象的唯一标识
        next()：返回一个可迭代数据结构（如列表）中的下一项
        print("..",end=""):end="" 关闭默认（换行）行为。
    内置函数（built-in functions,BIF）：会自动导入到__main__命名空间，但是这些BIF有它们自己的命名空间——__bultins__
